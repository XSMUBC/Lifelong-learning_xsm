...
class AutoEncoder(Replayer):
    ...
    def decode(self, z):
        ...
        image_tensor=image_recon[0,0, :,:]

        timestr = time.strftime("%Y%m%d-%H%M%S")
        plt.imsave('./plots/'+timestr+'.png', image_tensor.detach().numpy(), cmap='gray')
    ...
    def sample(self, size, dnclen, batch_index,z0,task,tasks,t_label,impor,pi,model,task_id):
        ...
        if dnclen:  # a new form of gaussian distribution:more accurate, more sharp, faster
	        ...
	        z,t_label,impor=maindnc(self, size, batch_index,z0,task,tasks,t_label,impor,pi,model,task_id)
